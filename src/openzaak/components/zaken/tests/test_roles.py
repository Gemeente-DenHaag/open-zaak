...
@tag("api_roles")
@override_settings(ALLOWED_HOSTS=["testserver"])
class IncorrectRoleTests(JWTAuthMixin, TypeCheckMixin, APITestCase):
    scopes = [SCOPE_ZAKEN_ALLES_LEZEN, SCOPE_ZAKEN_BIJWERKEN]
    component = ComponentTypes.zrc
    max_vertrouwelijkheidaanduiding = VertrouwelijkheidsAanduiding.confidentieel
    ...
    def test_zaakeigenschappen_list_incorrect_role(self):
        """
        Using an incorrect role as the only role should result in no access
        """
        zaaktype_url = f"http://testserver{reverse(self.zaaktype)}"
